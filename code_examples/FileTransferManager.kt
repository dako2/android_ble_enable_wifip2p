/**
 * File Transfer Manager - Handles media file transfer via WiFiP2P
 * Integrated with GlassesSDKSample's BLE approach
 */

package com.sdk.glassessdksample.transfer
    ...
    private fun startFileTransferProtocol() {
        networkExecutor.execute {
            try {
                ...
                val request = byteArrayOf(CMD_REQUEST_FILE_LIST)
    ...
    private fun requestFile(fileIndex: Int) {
        networkExecutor.execute {
            try {
                ...
                // Send file request
                val request = byteArrayOf(CMD_REQUEST_FILE, fileIndex.toByte())
                output.write(request)
                output.flush()
