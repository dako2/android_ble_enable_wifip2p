...
final class ConfigurableMutableNetwork<N, E> extends ConfigurableNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair endpointPairM447of = EndpointPair.m447of(this, n, n2);
            Preconditions.checkArgument(endpointPairIncidentNodes.equals(endpointPairM447of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, endpointPairIncidentNodes, endpointPairM447of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m444of();
            }
            return DirectedNetworkConnections.m445of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m450of();
        }
        return UndirectedNetworkConnections.m451of();
    }
}
