...
class EngineJob<R> implements DecodeJob.Callback<R>, FactoryPools.Poolable {
    ...
    void notifyCallbacksOfResult() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ...
                next.executor.execute(new CallResourceReady(next.f114cb));
            }
    ...
    void notifyCallbacksOfException() {
        synchronized (this) {
            ...
            while (it.hasNext()) {
                ResourceCallbackAndExecutor next = it.next();
                next.executor.execute(new CallLoadFailed(next.f114cb));
            }
            decrementPendingCallbacks();
        }
    }
    ...
    private class CallLoadFailed implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f112cb;
        ...
        public void run() {
            synchronized (this.f112cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f112cb)) {
                        EngineJob.this.callCallbackOnLoadFailed(this.f112cb);
                    }
    ...
    private class CallResourceReady implements Runnable {
        ...
        /* renamed from: cb */
        private final ResourceCallback f113cb;

        CallResourceReady(ResourceCallback resourceCallback) {
            this.f113cb = resourceCallback;
        }

        @Override // java.lang.Runnable
        public void run() {
            synchronized (this.f113cb.getLock()) {
                synchronized (EngineJob.this) {
                    if (EngineJob.this.cbs.contains(this.f113cb)) {
                        EngineJob.this.engineResource.acquire();
                        EngineJob.this.callCallbackOnResourceReady(this.f113cb);
                        EngineJob.this.removeCallback(this.f113cb);
                    }
                    EngineJob.this.decrementPendingCallbacks();
                }
            }
    ...
    static final class ResourceCallbackAndExecutor {
        ...
        /* renamed from: cb */
        final ResourceCallback f114cb;
        final Executor executor;

        ResourceCallbackAndExecutor(ResourceCallback resourceCallback, Executor executor) {
            this.f114cb = resourceCallback;
            this.executor = executor;
        }

        public boolean equals(Object obj) {
            if (obj instanceof ResourceCallbackAndExecutor) {
                return this.f114cb.equals(((ResourceCallbackAndExecutor) obj).f114cb);
            }
            return false;
        }

        public int hashCode() {
            return this.f114cb.hashCode();
        }
    }
