...
final class PermissionChecker {
    ...
    static void checkMediaLocationPermission(Context context, List<String> list) {
        if (PermissionUtils.containsPermission(list, Permission.ACCESS_MEDIA_LOCATION)) {
            ...
            while (it.hasNext()) {
                if (!PermissionUtils.containsPermission(new String[]{Permission.ACCESS_MEDIA_LOCATION, Permission.READ_MEDIA_IMAGES, Permission.READ_MEDIA_VIDEO, Permission.READ_EXTERNAL_STORAGE, Permission.WRITE_EXTERNAL_STORAGE, Permission.MANAGE_EXTERNAL_STORAGE}, it.next())) {
    ...
    static void checkLocationPermission(List<String> list) {
        if (PermissionUtils.containsPermission(list, Permission.ACCESS_BACKGROUND_LOCATION)) {
            ...
            Iterator<String> it = list.iterator();
            while (it.hasNext()) {
                if (!PermissionUtils.containsPermission(new String[]{Permission.ACCESS_FINE_LOCATION, Permission.ACCESS_COARSE_LOCATION, Permission.ACCESS_BACKGROUND_LOCATION}, it.next())) {
    ...
    static void checkNearbyDevicesPermission(List<String> list, AndroidManifestInfo androidManifestInfo) {
        ...
        for (AndroidManifestInfo.PermissionInfo permissionInfo : androidManifestInfo.permissionInfoList) {
            if (PermissionUtils.containsPermission(new String[]{Permission.BLUETOOTH_SCAN, Permission.NEARBY_WIFI_DEVICES}, permissionInfo.name) && !permissionInfo.neverForLocation()) {
    ...
    static void checkTargetSdkVersion(Context context, List<String> list) {
        int iFindAndroidVersionByPermission;
        for (String str : list) {
            if (PermissionUtils.equalsPermission(str, Permission.READ_MEDIA_VISUAL_USER_SELECTED)) {
                iFindAndroidVersionByPermission = 33;
            } else {
                iFindAndroidVersionByPermission = PermissionUtils.containsPermission(new String[]{Permission.BLUETOOTH_SCAN, Permission.BLUETOOTH_CONNECT, Permission.BLUETOOTH_ADVERTISE}, str) ? 23 : PermissionHelper.findAndroidVersionByPermission(str);
            }
            if (AndroidVersion.getTargetSdkVersionCode(context) < iFindAndroidVersionByPermission) {
                throw new IllegalStateException("Request " + str + " permission, The targetSdkVersion SDK must be " + iFindAndroidVersionByPermission + " or more, if you do not want to upgrade targetSdkVersion, please apply with the old permission");
            }
        }
    }
    ...
    static void checkManifestPermissions(Context context, List<String> list, AndroidManifestInfo androidManifestInfo) {
        ...
        for (String str : list) {
            if (!PermissionHelper.isVirtualPermission(str)) {
                ...
                if (PermissionUtils.equalsPermission(str, Permission.BODY_SENSORS_BACKGROUND)) {
                    ...
                    if (i >= PermissionHelper.findAndroidVersionByPermission(str)) {
                        return;
                    }
