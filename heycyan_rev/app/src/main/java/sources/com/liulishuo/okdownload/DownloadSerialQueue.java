...
public class DownloadSerialQueue extends DownloadListener2 implements Runnable {
    ...
    public synchronized void pause() {
        if (this.paused) {
            Util.m586w(TAG, "require pause this queue(remain " + this.taskList.size() + "), butit has already been paused");
            return;
        }
        this.paused = true;
    ...
    public synchronized void resume() {
        if (!this.paused) {
            Util.m586w(TAG, "require resume this queue(remain " + this.taskList.size() + "), but it is still running");
            return;
        }
        this.paused = false;
