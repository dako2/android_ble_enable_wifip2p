...
public class FileLock {
    ...
    public void increaseLock(String str) {
        ...
        Util.m583d(TAG, "increaseLock increase lock-count to " + atomicInteger.incrementAndGet() + str);
    }
    ...
    public void decreaseLock(String str) {
        ...
        Util.m583d(TAG, "decreaseLock decrease lock-count to 0 " + str);
        ...
        if (thread != null) {
            Util.m583d(TAG, "decreaseLock " + str + " unpark locked thread " + atomicInteger);
            unpark(thread);
        }
        synchronized (this.fileLockCountMap) {
    ...
    public void waitForRelease(String str) {
        ...
        Util.m583d(TAG, "waitForRelease start " + str);
        while (!isNotLocked(atomicInteger)) {
            park();
        }
        Util.m583d(TAG, "waitForRelease finish " + str);
    }
