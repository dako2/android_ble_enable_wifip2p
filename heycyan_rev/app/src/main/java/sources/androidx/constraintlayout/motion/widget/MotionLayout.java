    ...
    */
    ...
    class Model {
        ...
        private void debugLayout(String str, ConstraintWidgetContainer constraintWidgetContainer) {
            String str2 = str + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + Debug.getName((View) constraintWidgetContainer.getCompanionWidget());
            ...
            for (int i = 0; i < size; i++) {
                ...
                Log.v(MotionLayout.TAG, str3 + "  " + name + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + constraintWidget + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + str4);
            }
    ...
    private void debugPos() {
        for (int i = 0; i < getChildCount(); i++) {
            View childAt = getChildAt(i);
            Log.v(TAG, HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + Debug.getLocation() + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + Debug.getName(this) + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + Debug.getName(getContext(), this.mCurrentState) + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + Debug.getName(childAt) + childAt.getLeft() + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + childAt.getTop());
        }
    }
    ...
    */
    ...
    private void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            TypedArray typedArrayObtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, C0171R.styleable.MotionLayout);
            int indexCount = typedArrayObtainStyledAttributes.getIndexCount();
            boolean z = true;
            for (int i = 0; i < indexCount; i++) {
                int index = typedArrayObtainStyledAttributes.getIndex(i);
                if (index == C0171R.styleable.MotionLayout_layoutDescription) {
                    this.mScene = new MotionScene(getContext(), this, typedArrayObtainStyledAttributes.getResourceId(index, -1));
                } else if (index == C0171R.styleable.MotionLayout_currentState) {
                    this.mCurrentState = typedArrayObtainStyledAttributes.getResourceId(index, -1);
                } else if (index == C0171R.styleable.MotionLayout_motionProgress) {
                    this.mTransitionGoalPosition = typedArrayObtainStyledAttributes.getFloat(index, 0.0f);
                    this.mInTransition = true;
                } else if (index == C0171R.styleable.MotionLayout_applyMotionScene) {
                    z = typedArrayObtainStyledAttributes.getBoolean(index, z);
                } else if (index == C0171R.styleable.MotionLayout_showPaths) {
                    if (this.mDebugPath == 0) {
                        this.mDebugPath = typedArrayObtainStyledAttributes.getBoolean(index, false) ? 2 : 0;
                    }
                } else if (index == C0171R.styleable.MotionLayout_motionDebug) {
    ...
    public void updateStateAnimate(int i, ConstraintSet constraintSet, int i2) {
        if (this.mScene != null && this.mCurrentState == i) {
            updateState(C0171R.id.view_transition, getConstraintSet(i));
            setState(C0171R.id.view_transition, -1, -1);
            updateState(i, constraintSet);
            MotionScene.Transition transition = new MotionScene.Transition(-1, this.mScene, C0171R.id.view_transition, i);
