...
class FragmentStateManager {
    ...
    int computeExpectedState() {
        ...
        int i = C04012.$SwitchMap$androidx$lifecycle$Lifecycle$State[this.mFragment.mMaxState.ordinal()];
        if (i != 1) {
            if (i == 2) {
    ...
    void resume() {
        ...
        if (focusedView != null && isFragmentViewChild(focusedView)) {
            ...
            if (FragmentManager.isLoggingEnabled(2)) {
                Log.v("FragmentManager", "requestFocus: Restoring focused view " + focusedView + HelpFormatter.DEFAULT_LONG_OPT_SEPARATOR + (zRequestFocus ? "succeeded" : "failed") + " on Fragment " + this.mFragment + " resulting in focused view " + this.mFragment.mView.findFocus());
            }
        }
