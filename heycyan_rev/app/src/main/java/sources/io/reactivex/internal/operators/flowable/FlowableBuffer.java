...
public final class FlowableBuffer<T, C extends Collection<? super T>> extends AbstractFlowableWithUpstream<T, C> {
    ...
    static final class PublisherBufferExactSubscriber<T, C extends Collection<? super T>> implements Subscriber<T>, Subscription {
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                this.f594s.request(BackpressureHelper.multiplyCap(j, this.size));
            }
        }
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f594s, subscription)) {
                this.f594s = subscription;
    ...
    static final class PublisherBufferSkipSubscriber<T, C extends Collection<? super T>> extends AtomicInteger implements Subscriber<T>, Subscription {
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                if (get() == 0 && compareAndSet(0, 1)) {
                    this.f596s.request(BackpressureHelper.addCap(BackpressureHelper.multiplyCap(j, this.size), BackpressureHelper.multiplyCap(this.skip - this.size, j - 1)));
                    return;
                }
                this.f596s.request(BackpressureHelper.multiplyCap(this.skip, j));
            }
        }
        ...
        @Override // org.reactivestreams.Subscriber
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f596s, subscription)) {
                this.f596s = subscription;
                this.actual.onSubscribe(this);
            }
        }
    ...
    static final class PublisherBufferOverlappingSubscriber<T, C extends Collection<? super T>> extends AtomicLong implements Subscriber<T>, Subscription, BooleanSupplier {
        ...
        public void request(long j) {
            ...
            if (!this.once.get() && this.once.compareAndSet(false, true)) {
                this.f595s.request(BackpressureHelper.addCap(this.size, BackpressureHelper.multiplyCap(this.skip, j - 1)));
            } else {
                this.f595s.request(BackpressureHelper.multiplyCap(this.skip, j));
            }
        }
        ...
        @Override // org.reactivestreams.Subscriber
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f595s, subscription)) {
                this.f595s = subscription;
                this.actual.onSubscribe(this);
            }
        }
