...
public abstract class Flowable<T> implements Publisher<T> {
    ...
    public final <R> Single<R> reduce(R r, BiFunction<R, ? super T, R> biFunction) {
        return RxJavaPlugins.onAssembly(new FlowableSingleSingle(scan(r, biFunction).takeLast(1), null));
    }

    @SchedulerSupport(SchedulerSupport.NONE)
    @BackpressureSupport(BackpressureKind.UNBOUNDED_IN)
    public final <R> Single<R> reduceWith(Callable<R> callable, BiFunction<R, ? super T, R> biFunction) {
        return RxJavaPlugins.onAssembly(new FlowableSingleSingle(scanWith(callable, biFunction).takeLast(1), null));
    }
    ...
    public final Flowable<T> sample(long j, TimeUnit timeUnit, Scheduler scheduler) {
        ObjectHelper.requireNonNull(timeUnit, "unit is null");
        ObjectHelper.requireNonNull(scheduler, "scheduler is null");
        return RxJavaPlugins.onAssembly(new FlowableSampleTimed(this, j, timeUnit, scheduler));
    }
    ...
    public final <U> Flowable<T> sample(Publisher<U> publisher) {
        ...
        return RxJavaPlugins.onAssembly(new FlowableSamplePublisher(this, publisher));
    }
    ...
    @BackpressureSupport(BackpressureKind.SPECIAL)
    /* renamed from: to */
    public final <R> R m600to(Function<? super Flowable<T>, R> function) {
        try {
