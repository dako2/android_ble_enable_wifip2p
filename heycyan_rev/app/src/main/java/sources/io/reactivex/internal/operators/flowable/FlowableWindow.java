...
public final class FlowableWindow<T> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    static final class WindowExactSubscriber<T> extends AtomicInteger implements Subscriber<T>, Subscription, Runnable {
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f689s, subscription)) {
                this.f689s = subscription;
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                this.f689s.request(BackpressureHelper.multiplyCap(this.size, j));
            }
        }
    ...
    static final class WindowSkipSubscriber<T> extends AtomicInteger implements Subscriber<T>, Subscription, Runnable {
        ...
        @Override // org.reactivestreams.Subscriber
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.f691s, subscription)) {
                this.f691s = subscription;
                this.actual.onSubscribe(this);
            }
        }
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                if (!this.firstRequest.get() && this.firstRequest.compareAndSet(false, true)) {
                    this.f691s.request(BackpressureHelper.addCap(BackpressureHelper.multiplyCap(this.size, j), BackpressureHelper.multiplyCap(this.skip - this.size, j - 1)));
                } else {
                    this.f691s.request(BackpressureHelper.multiplyCap(this.skip, j));
                }
            }
        }
    ...
    static final class WindowOverlapSubscriber<T> extends AtomicInteger implements Subscriber<T>, Subscription, Runnable {
        ...
        public void request(long j) {
            if (SubscriptionHelper.validate(j)) {
                ...
                if (!this.firstRequest.get() && this.firstRequest.compareAndSet(false, true)) {
                    this.f690s.request(BackpressureHelper.addCap(this.size, BackpressureHelper.multiplyCap(this.skip, j - 1)));
                } else {
                    this.f690s.request(BackpressureHelper.multiplyCap(this.skip, j));
                }
                drain();
            }
        }
