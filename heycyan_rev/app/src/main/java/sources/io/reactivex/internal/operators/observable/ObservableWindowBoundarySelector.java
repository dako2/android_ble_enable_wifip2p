...
public final class ObservableWindowBoundarySelector<T, B, V> extends AbstractObservableWithUpstream<T, Observable<T>> {
    ...
    static final class WindowBoundaryMainObserver<T, B, V> extends QueueDrainObserver<T, Object, Observable<T>> implements Disposable {
        ...
        /* renamed from: ws */
        final List<UnicastSubject<T>> f837ws;
        ...
        WindowBoundaryMainObserver(Observer<? super Observable<T>> observer, ObservableSource<B> observableSource, Function<? super B, ? extends ObservableSource<V>> function, int i) {
            ...
            this.f837ws = new ArrayList();
            atomicLong.lazySet(1L);
        }

        @Override // io.reactivex.Observer
        public void onSubscribe(Disposable disposable) {
            if (DisposableHelper.validate(this.f836s, disposable)) {
                this.f836s = disposable;
                this.actual.onSubscribe(this);
                if (this.cancelled) {
                    return;
                }
                OperatorWindowBoundaryOpenObserver operatorWindowBoundaryOpenObserver = new OperatorWindowBoundaryOpenObserver(this);
                if (LifecycleKt$$ExternalSyntheticBackportWithForwarding0.m47m(this.boundary, null, operatorWindowBoundaryOpenObserver)) {
        ...
        public void onNext(T t) {
            if (fastEnter()) {
                Iterator<UnicastSubject<T>> it = this.f837ws.iterator();
                while (it.hasNext()) {
                    it.next().onNext(t);
                }
                if (leave(-1) == 0) {
        ...
        void drainLoop() {
            ...
            List<UnicastSubject<T>> list = this.f837ws;
            int iLeave = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (!z2) {
                    if (objPoll instanceof WindowOperation) {
                        ...
                        if (windowOperation.f838w != null) {
                            if (list.remove(windowOperation.f838w)) {
                                windowOperation.f838w.onComplete();
                                if (this.windows.decrementAndGet() == 0) {
                                    disposeBoundary();
                                    return;
                                }
        ...
        void close(OperatorWindowBoundaryCloseObserver<T, V> operatorWindowBoundaryCloseObserver) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseObserver.f835w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f838w;

        WindowOperation(UnicastSubject<T> unicastSubject, B b) {
            this.f838w = unicastSubject;
            this.open = b;
        }
    }
    ...
    static final class OperatorWindowBoundaryCloseObserver<T, V> extends DisposableObserver<V> {
        ...
        /* renamed from: w */
        final UnicastSubject<T> f835w;

        OperatorWindowBoundaryCloseObserver(WindowBoundaryMainObserver<T, ?, V> windowBoundaryMainObserver, UnicastSubject<T> unicastSubject) {
            this.parent = windowBoundaryMainObserver;
            this.f835w = unicastSubject;
        }
